...
USING 
  (SELECT DISTINCT
    *, 
    ...
    True as is_active
  FROM `chetan-retailer-project-454413.bronze.customers`) source
...
USING 
  (SELECT DISTINCT
    *, 
    ...
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `chetan-retailer-project-454413.bronze.orders`) source
...
CREATE TABLE IF NOT EXISTS `chetan-retailer-project-454413.silver.order_items`
(
    order_item_id INT64,
    order_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `chetan-retailer-project-454413.silver.order_items` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `chetan-retailer-project-454413.bronze.order_items`) source
...
CREATE TABLE IF NOT EXISTS `chetan-retailer-project-454413.silver.product_suppliers`
(
    supplier_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `chetan-retailer-project-454413.silver.product_suppliers` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `chetan-retailer-project-454413.bronze.product_suppliers`) source
...
CREATE TABLE IF NOT EXISTS `chetan-retailer-project-454413.silver.customer_reviews`
(
    id STRING,
    customer_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `chetan-retailer-project-454413.silver.customer_reviews` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `chetan-retailer-project-454413.bronze.customer_reviews`) source
